...
def table():
    ...
    X = torch.randn(BATCH, 12, 5)
    y_train = torch.randint(0, 3, (BATCH, 8)).float()
...
class TestLimixAdapterIntegration:
    ...
    def test_capture_one_4d_tap_per_depth(self, limix_adapter, table):
        X, y, eval_pos = table
    ...
    def test_skip_layer_is_identity(self, limix_adapter, table):
        X, y, eval_pos = table
